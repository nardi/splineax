...
def _assert_superseded_rebuild(
    ...
    solve's `solve_with_numeric` still names the pre-refactor one. With no trace open
...
def test_two_matrix_solve_reuses_analysis_under_jit() -> None:
    ...
    # The trace itself forces solve-before-refactor ordering, so rebuilds are counted
    # on a fresh untraced call of the same jitted function.
