        ...
        match operator:
            ...
            case _:
                raise TypeError(
                    ...
                    f"`lineax.FunctionLinearOperator`; got {type(operator).__name__}."
                )
...
class Pardiso(AbstractSparseLinearSolver[_PardisoState]):
    ...
    def factorize_symbolic(
        ...
        Args:
            sparsity: Sparse matrix whose sparsity pattern to pre-analyze. Accepts the
                      ...
                      The scope also keeps this object itself, and colors it (or
                      reuses the coloring it carries) to sparsely materialise any
                      dense `lineax.JacobianLinearOperator` or
    ...
    def _factorize_symbolic(
        ...
        with _PardisoHandleAllocationScopeManager.begin_scope():
            ...
            handle = _PardisoHandleAllocationScopeManager.register_handle(value)
            yield _PardisoSymbolicScope(tuple(shape), handle, sparsity)
